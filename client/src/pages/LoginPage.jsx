...
export default function LoginPage({ me, onLogin }) {
  ...
  if (!me.loading && me.loggedIn) {
    const defaultPath = me.user?.role === 'admin' ? '/admin' : me.user?.role === 'teacher' ? '/teacher' : '/test';
    const fromPath = location.state?.from?.pathname || defaultPath;
    return <Navigate to={fromPath} replace />;
  }

  return (
    <div className="page">
      <Breadcrumbs items={[{ to: '/', label: 'Басты бет' }, { label: 'Кіру' }]} />
        ...
        <LoginForm
          ...
          onSubmit={async (values) => {
            ...
            try {
              ...
              // After login, `me.loggedIn` becomes true and the component re-renders
              // and the `Navigate` block at the top executes.
            } finally {
              setBusy(false);
            }
          }}
